...
import events_illustration from '../images/events_illustration.png' 
...
class EventSection extends Component {
    ...
    render(){
        const noEventsMessage = "There are no events scheduled at this time. Check back regularly as we're always working on new ideas."
        ...
        if(this.state.upcomingEvents.length === 0){
            ...
            return(
                <>
                <Container id="events" className="pt-2 px-5 mb-0">
                    <h1 className="display-4 section-title mb-3">Upcoming Events</h1>
                    <Row className="justify-content-between pb-3">
                        <Col sm={12} lg={5} className="mb-5 event-illustration">
                            <Image src={events_illustration} alt="events illustration" fluid />
                        </Col>
                        <Col sm={12} lg={7}>
                            <p className="lead mt-5 mb-4 no-event-message" style={{fontSize: "1.2rem"}}>{noEventsMessage}</p>
                            <Button variant="outline-info" href={pastEvents} target="_blank"><Calendar4 className="mr-2 calendar-icon" /> view Past Events</Button>
                        </Col> 
                    </Row>
                </Container>
            </>
            ...
            <>
                <Container id="events" className="pt-2 px-5">
                    ...
                    <div className="text-center my-3">
                        <Button variant="outline-secondary my-3" href={pastEvents} target="_blank"> <Calendar4 className="mr-2 calendar-icon" /> view Past Events</Button>
                    </div>
                </Container>
            </>
        );
